  ...
  async analyzeInvoice(file: string) {
    ...
    const process_text_detection = async () => {
      try {
        ...
        // const filteredResponse = response.ExpenseDocuments[0].LineItemGroups[0].LineItems.filter((item=>item.Type.Text !=="OTHER"))
        response.ExpenseDocuments.forEach(exd=>{
          exd.LineItemGroups.forEach(lg=>{
            lg.LineItems.forEach(lt=>{
              lt.LineItemExpenseFields =  lt.LineItemExpenseFields.filter(lxf=>{
                lxf.Type.Text !== "OTHER"
              })
            })
          })
        })
        fs.writeFile('invoiceData.json', JSON.stringify(response), function (err) {
          if (err) {
            console.log(err);
          }
        });
